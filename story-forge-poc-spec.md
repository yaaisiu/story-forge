...
- Every LLM operation records: model, input_tokens / output_tokens **whenever the provider returns them** (including Ollama's `prompt_eval_count` / `eval_count` — kept, not discarded), cost_estimate (nullable; paid tiers), GPU-seconds (nullable; Ollama Cloud's billing unit), and latency_ms (the elapsed time around the provider call; recorded for every *dispatched* call — success, failover failure, or a provider-side refusal — and null only for a row written *before* any provider call, e.g. a fail-closed budget refusal at the gate, which has nothing to time) — one usage row per call, including refusals and failures so the trail explains *why* a batch stopped. Tier/provider/model are system-derived from the adapter that served the call, never echoed from the caller (so the cost ledger can't be lied to).
...
- **Dependencies pinned and minimum 14 days old.** Every package in `pyproject.toml` / `package.json` pinned to an exact version. No version is selected that was released less than 14 days ago — gives time for malicious-package alerts to surface. CI check enforces this. _(Direct-URL wheel channel, added 2026-05-29 / M2.S1: a few dependencies are not published on PyPI/npm — notably spaCy's pretrained pipeline packages `pl_core_news_lg` and `en_core_web_lg`, distributed only as versioned wheels on GitHub Releases. These are pinned as PEP 508 direct-URL references — `name @ https://github.com/<org>/<repo>/releases/download/<tag>/<name>-<version>-…whl` — which are **exact by construction** (a release-asset URL is immutable and carries the version) and **hash-locked** in `uv.lock` (uv records and verifies a SHA-256 for URL wheels). The 14-day soak still applies, enforced against the **matching release asset's upload timestamp** (its `updated_at`) instead of a PyPI upload date: `check_dependency_age.py` resolves the release through the GitHub API, finds the asset whose filename matches the locked wheel, and applies the same cutoff. Asset timestamp, *not* the tag's publish date — a wheel can be added to or replaced on an older release after the tag was cut, and since uv hash-locks whatever bytes we pin, the tag date would let a freshly-uploaded artifact slip past the soak. The OSV/advisory gate is the one rule that does **not** apply — these artifacts are not indexed by OSV/pip-audit — so the residual supply-chain risk is bounded instead by: (a) the official publisher (Explosion's `spacy-models` org), (b) the locked SHA-256 guaranteeing artifact integrity on every install, and (c) the wheel carrying only the standard spaCy pipeline (model weights + config), not novel network/exec code. This adapts — does not relax — the exact-pin and 14-day rules; see `scripts/check_dependency_age.py` GitHub-release handling and the `/add-dependency` skill.)_ _(HuggingFace-model channel, **policy defined 2026-06-11 / M3.S1**; the concrete model pin lands with the Stage-2 embedding work in **M3.S2**. The Stage-2 embedding model — `paraphrase-multilingual-mpnet-base-v2` (768-dim, multilingual PL/EN, the spec §3.3 example) — ships on the HuggingFace Hub, not PyPI, and sentence-transformers' default is to fetch it from the Hub at first use — exactly the unpinned runtime fetch this rule forbids (the same hole the spaCy-wheel channel above closed). The channel pins it by the model repo's **immutable commit revision SHA**: a one-off `huggingface_hub.snapshot_download(repo_id, revision=<full-40-char-SHA>)` step (in an optional dependency group / a pin script, not a default runtime download) resolves to content-addressed bytes that cannot change under a fixed revision, the integrity analogue of a locked wheel SHA-256. The runtime **library** stack — `sentence-transformers`, `torch`, `huggingface_hub` — are ordinary exact PyPI pins under the 14-day rule + OSV gate (note: `torch`'s `torch.jit.script` advisory affects all versions with no fix and is unreachable here — inference-only `encode`, no `jit.script` — so it is handled by a scoped `infra/osv/` waiver, not a version bump). The 14-day soak for the **model artifact** is verified at pin time against the pinned commit's Hub date (recorded with the SHA where it is pinned) — a procedural check like the image-tag soak, since the model is not in `uv.lock` for `check_dependency_age.py` to see. OSV/advisory scanning is **N/A** for the weights (not indexed, same as the spaCy wheels); residual supply-chain risk is bounded by (a) the official publisher (`sentence-transformers` org), (b) the immutable revision SHA guaranteeing byte-for-byte reproducibility, and (c) the artifact carrying only model weights + tokenizer config, not novel network/exec code. This adapts — does not relax — the exact-pin and soak rules to a registry-less artifact.)_
- **Container images pinned, aged, and CVE-scanned.** Every image in `docker-compose.yml` is pinned to an exact tag (no `latest`, no floating majors), the tag is **at least 7 days old** at time of pin, and the image is verified CVE-clean against an OSS scanner (Trivy) before being committed. A CI job re-runs the scan on every code-bearing push/PR and on a daily schedule (see *CI scan cadence* below) so regressions are caught. _(The image soak is 7 days, shorter than the 14-day package rule, because base images come from known, signed official publishers — the dominant image risk is known CVEs, not a hijacked release, and for CVEs freshness *reduces* exposure (a newer rebuild ships patched OS packages). Trivy CVE-cleanliness is therefore the primary gate; a 7-day soak still leaves room for a compromised-tag alert to surface. Bundled-dependency CVEs that no tag/base variant can fix are waived with per-CVE justification in a scoped `.trivyignore`, dropped when upstream rebuilds.)_
...
- **Frontend dependencies audited.** `npm audit` runs in CI; high/critical issues fail the build. _(Scope clarified 2026-06-18 / M4.S1: the gate was narrowed to `npm audit --omit=dev` at high/critical (then spelled `--audit-level=high`; see the 2026-07-31 note below for how it is invoked now) — it audits the **shipped/runtime** dependencies, not the build/test toolchain (`devDependencies` like `jsdom`, `vitest`, `eslint`, the Vite plugins). Rationale: the frontend ships a static SPA bundle, so a dev-only dependency is never delivered to a user and cannot reach the runtime trust surface — auditing it produces noise that does not reflect shipped risk (the standard SPA posture). Runtime deps — React, react-router-dom, TanStack Query, etc. — stay gated at high/critical. This **adapts, not relaxes** the control: it narrows the gate to what actually ships, consistent with the threat model (single user, localhost, no public surface). Trigger: a HIGH `undici` advisory (GHSA-vmh5-mc38-953g) reached us only transitively through `jsdom` (the test DOM env) with no 14-day-soaked fix, while being unreachable in this deployment — the dev-tool noise the prod-scoped gate is designed to exclude. A dev-tool advisory is still visible to a plain `npm audit` and to GitHub Dependabot; the gate just doesn't block `main` on un-shipped code.)_ _(**Waiver mechanism added 2026-07-31 / Grzymalin S5.** The gate now runs `python3 scripts/check_npm_audit.py`, a thin wrapper over `npm audit --omit=dev --json` — **the enforced threshold is unchanged**: any unwaived HIGH/CRITICAL in a shipped dependency still fails the build. What it adds is the ability to waive an **assessed-unreachable** advisory **scoped, never gate-wide**, in `infra/npm/audit-waivers.toml` (the enforced home) with the rationale + a dated drop-when recorded in the `infra/npm/WAIVERS.md` register — the same scoped-ignore + human-register split the Trivy image scans and the backend OSV SCA already use. Every waiver carries a mandatory `ignoreUntil` date, so an ignore **re-reds automatically** when it expires rather than rotting behind a green board (the `osv-scanner.toml` `ignoreUntil` analogue), and the script reports a waiver whose advisory is no longer present as **stale** so retired entries get deleted. For that expiry to be real the gate runs in the **`security`** CI job — which force-runs on the daily schedule — and *not* in the path-scoped `frontend` job, where a docs-only stretch past the date would leave a lapsed ignore silently suppressed. A dated waiver mechanism and a path-scoped job are incompatible; any future advisory gate belongs in `security` for the same reason. Rationale: `npm audit` was the **one §6.7 gate with no waiver mechanism at all**, so an advisory that is both unreachable here and unfixable could only be handled by relaxing the gate wholesale to `--audit-level=critical` — which would stop gating *every* future HIGH, strictly worse than a scoped, dated, justified exception. This **adapts, not relaxes** the control. Trigger: `GHSA-qwww-vcr4-c8h2` (React Router, HIGH — RSC-mode CSRF bypass) is patched **only** in `react-router` 8.3.0 while `react-router-dom` has no v8 release, so **no soaked forward bump can clear it**; the advisory states it "only affects your application if you are using the unstable RSC APIs", and this frontend is a Vite SPA with no SSR and zero RSC usage. Fix-first still governs — prefer a bump via `/add-dependency` over a waiver, and `/triage-advisory` owns the whole waiver lifecycle including the drop-revisit. **Trigger overtaken 2026-08-13 / Grzymalin Session 107 — the mechanism is unaffected, the worked example is not.** The "no soaked forward bump can clear it" claim above was correct and empirically verified when written on 2026-07-31 (both `7.18.1` and `7.18.2` were installed in a scratch project and still reported the advisory). The advisory was then **amended upstream** to record a 7.x backport, moving the vulnerable range from `>= 7.12.0, < 8.3.0` to `>= 7.12.0, < 7.18.2`, so `react-router-dom 7.18.1 → 7.18.2` cleared it outright and the waiver was dropped by a **fix**, 18 days before its `ignoreUntil` (PR #236). **Nothing in this repo changed.** The original sentence is left standing rather than rewritten, because a security exception should record the real event that motivated it, not a tidied-up one. Two things follow, and both *strengthen* the design above rather than qualify it: (1) an advisory's "no fix exists" is a **point-in-time fact, not a permanent property** — a rationale that forecloses a fix is the most dangerous kind, because it reads as a reason to stop looking; (2) the mandatory `ignoreUntil` bounds how long an ignore can rot but **cannot detect a fix arriving early** — here a Dependabot cross-check did, since Dependabot tracks each advisory's *current* patched range while our gate audits the *installed* version and reported "waived" faithfully. `/triage-advisory` carries both lessons (a Dependabot enumeration in step 1; re-verifying a recorded "no fix exists" in step 3).)_
- **Backend dependencies advisory-scanned (continuous SCA).** A CI step runs `osv-scanner` against `backend/uv.lock` on every code-bearing push/PR and on a daily schedule (see *CI scan cadence* below) and **fails the build on any known advisory** (fail-on-any, not just HIGH/CRITICAL — the triggering case, `starlette` 1.0.0 / GHSA-86qp-5c8j-p5mr, was a MEDIUM a HIGH/CRITICAL-only gate would have missed). This is **software-composition analysis** (the PyPI analogue of `npm audit`): it closes the gap where a vulnerability disclosed *after* a package is pinned is invisible to the freshness/soak rule. It is distinct from, and the continuous complement to, the **pin-time** OSV check `/add-dependency` already runs, and it is defense-in-depth *with* — not a replacement for — GitHub's server-side Dependabot. The scanner is itself a supply-chain surface, so it is pinned by **immutable image digest** (`ghcr.io/google/osv-scanner@sha256:…`, ≥7-day soak as for images) rather than a floating tag or a GitHub Action. Assessed-but-accepted advisories are waived **scoped, never repo-wide**, in `infra/osv/osv-scanner.toml` (the enforced home, wired via `--config`) with the rationale + condition-based "drop when a fixed version clears the 14-day soak" recorded in the `infra/osv/WAIVERS.md` register — the same scoped-ignore + human-register split the Trivy image scans use. Prefer a fix (bump via `/add-dependency`) over a waiver. _(Added 2026-06-08; baseline control, not a new invariant — consistent with how pinning, the soak, and Trivy image-scanning live as §6.7 rules.)_
- **CI scan cadence — path-scoped, plus a daily run (added 2026-06-25, repo went public).** With the repo now **public** (unlimited free GitHub Actions; the git history was pre-flight-checked clean of secrets before flipping), the **heavy** CI jobs — the service-container test suites, the frontend build, and the dependency/image scans (dependency-age, `osv-scanner`, Trivy) — run only when a push/PR changes **code-bearing** files (anything outside `docs/**`, `architecture/**`, and `*.md`). A docs-only change touches no dependency, image, or code, so those scans have nothing to catch — skipping them there trims noise without losing coverage. Two backstops keep the baseline whole: the `detect-secrets` **secret scan runs on _every_ push/PR** (docs included), and the dependency/image scans **also run on a daily schedule**, so a newly-disclosed advisory against an *unchanged* dependency (the CVE-treadmill) is caught regardless of PR cadence. This **adapts, not relaxes** the "CI enforces every rule" baseline — every code-bearing change is fully scanned before reaching `main`, and treadmill coverage moves from per-PR-incidental to guaranteed-daily.
...
- CI: lint, format, basic tests, dependency age check (≥14 days), `npm audit`, backend advisory scan (`osv-scanner` vs `backend/uv.lock`, fail-on-any — added 2026-06-08), secret scan, container-image CVE scan (Trivy) against the images in `docker-compose.yml` _(since 2026-06-25 the heavy jobs are path-scoped to code-bearing changes with a daily scheduled run; the secret scan still runs on every PR — see §6.7 "CI scan cadence")_

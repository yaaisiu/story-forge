...
- **Dependencies pinned and minimum 14 days old.** Every package in `pyproject.toml` / `package.json` pinned to an exact version. No version is selected that was released less than 14 days ago — gives time for malicious-package alerts to surface. CI check enforces this.
- **Container images pinned, aged, and CVE-scanned.** Every image in `docker-compose.yml` is pinned to an exact tag (no `latest`, no floating majors), the tag is at least 14 days old at time of pin, and the image is verified CVE-clean against an OSS scanner (Trivy) before being committed. A CI job re-runs the scan on every push so regressions are caught.

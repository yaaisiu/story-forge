...
- CI: lint, format, basic tests, dependency age check (≥14 days), `npm audit`, backend advisory scan (`osv-scanner` vs `backend/uv.lock`, fail-on-any — added 2026-06-08), secret scan, container-image CVE scan (Trivy) against the images in `docker-compose.yml`

...
- **Review model (changed 2026-06-08; Codex Desktop retired 2026-06-10; GitHub-wired Codex PR review retired 2026-06-24).** Your own **`/review-pr` is the primary review gate** — run it on your own work before every merge. There is now **no automated second reviewer** (the in-tree Codex Desktop agent and the GitHub-wired Codex PR review are both gone). Because there's no second reviewer, raise your own bar: for *substantive code* changes where a missed bug is costly, run the heavier multi-agent **`/code-review`** in addition to `/review-pr` — the PR-#36 lesson is that a second reviewer once caught two real bugs the single self-review pass missed, and that safety net is gone. *(The standing review-gate policy — whether `/code-review` becomes a required second gate for substantive code — is an open question to settle in a later session, deferred 2026-06-24 when Codex was retired.)* **Solo-review blind spot:** self-review structurally can't catch what you didn't think to check, so for PRs that **flip decision-state across many homes** (a proposal→accepted, a spec amendment, an ADR), don't trust recall — run the `/review-pr` §2 reconciliation sweep as an *explicit pass* (grep the proposal slug + status words across `INDEX.md` + both plan files; diff each decision against the task that implements it) **before** claiming clean. PR #39 is the cautionary case: the self-review asserted §2 clean and still left three stale homes for review to find.

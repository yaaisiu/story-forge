...
- **Next step:** **Session 2 — Upload + language detection + parsing.** First work step is a *failing* test (spec- and test-driven). First in-session decision: language-detection library — **`langdetect` vs `fasttext`** (grounded comparison at the task; both must satisfy the exact-pin ≥14-day package rule — use `/add-dependency`). Build `POST /stories/upload` (txt/md/docx, size+MIME validation, uploads sandboxed to a dedicated dir), parsing helpers (docx/md/txt → raw text + paragraph blocks) and a PL/EN detection helper in `domain/`, then persist `Story` (+ `Project`) via the Session 1 repo storing raw text + detected language.
- **Read before starting (Session 2):** this file; spec **§7** (ingest pipeline — upload→parse→detect order), **§6.3** (project structure / where `api/`, `domain/` live), **§6.7** "File uploads sandboxed" bullet; `backend/CLAUDE.md` (FastAPI patterns, "Running tests") and `backend/src/story_forge/CLAUDE.md` (domain-vs-adapter split).
- **Verify on disk:** `main` at `4cb92a2` (Issue #4, PR #6 — pgvector + ollama CVE-stale fix). Session 1 anchors present: `backend/alembic/versions/*` migration, `adapters/postgres_repo.py`, `domain/models.py`, `tests/conftest.py` test-DB fixture. Trivy waivers present: `infra/trivy/{neo4j,pgvector,ollama}.trivyignore` + the `WAIVERS.md` register. **No `api/stories.py` yet** (Session 2 creates it). `backend/.env` must define `TEST_DATABASE_URL` for the integration suite.
- **Last session ended:** 2026-05-21 — **Issue #4 closed** (PR #6, squash `4cb92a2`): pgvector `0.8.1-pg17-bookworm` → `0.8.2-pg17-trixie` (7-day-rule bend, pre-approved) and ollama `0.22.1` → `0.24.0`, both with scoped per-CVE waivers; added `infra/trivy/WAIVERS.md` review register. **`main` fully green (all four CI jobs incl. `security`).** Retro amended `/pin-image` (local-Trivy-unreadable → push-and-harvest-from-CI fallback; scan all images up front; record waivers in WAIVERS.md).
- **Open blocks/questions:** **Session 2 first decision — `langdetect` vs `fasttext`** (first work step is the failing test). Deferred: Session 3 auto-chunking threshold. No infra blockers — `security` is green.

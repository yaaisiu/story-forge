...
- **Next step:** [Issue #2](https://github.com/yaaisiu/story-forge/issues/2) — fix the invalid container image tags (`neo4j`, audit `ollama`) so `main`'s `security` CI goes green and `docker compose up` pulls all services. Infra hygiene, not a numbered M1 session; do it before Session 2 so `main` is green. **Session 2 (Upload + language detection + parsing) is queued right after** — see its block below for spec sections and the in-session `langdetect` vs `fasttext` decision.
- **Read before starting (Issue #2):** this file, Issue #2, spec §6.2 (deployment / Docker) and §6.7 (security baseline — pinned, ≥14 days, Trivy HIGH/CRITICAL), `docker-compose.yml`, `.github/workflows/ci.yml`, `infra/neo4j/`
- **Verify on disk:** `main` at the squashed Session 1 commit; `docker-compose.yml` neo4j tag is `5.26.25-community-bullseye` (**invalid** — `bullseye` was dropped at patch .25; valid 5.26.25 variants: `-community`, `-community-trixie`, `-community-ubi10`); `ci.yml` security job already uses `trivy-action@v0.36.0` (fixed in Session 1); pgvector tag `0.8.1-pg17-bookworm` is valid (unchanged). For Session 2 afterward: `backend/.env` must define `TEST_DATABASE_URL` (per `backend/CLAUDE.md` "Running tests").
- **Last session ended:** 2026-05-20 — Session 1 **merged to `main`** (PR #1, squash `ff95dc2`); private repo `yaaisiu/story-forge` created. `main` CI: backend / frontend / ollama-cloud-smoke green; **`security` red** pending Issue #2 (pre-existing M0 image-tag bug, surfaced on the first-ever CI run).
- **Open blocks/questions:** Issue #2 carries a possible **decision** — a valid Neo4j community image may surface real HIGH/CRITICAL CVEs under Trivy (`ignore-unfixed: true`); if so, choose accept-with-justification / different base variant / waiver. Session 2 decisions still deferred: **`langdetect` vs `fasttext`** (its first work step is a failing upload/parser test) and Session 3 chunking threshold.
...
- **[Issue #2](https://github.com/yaaisiu/story-forge/issues/2) — invalid container image tags (next step, before Session 2):** `main`'s `security` CI is red — `neo4j:5.26.25-community-bullseye` does not exist (bullseye dropped at patch .25), and the same dead tag is in `docker-compose.yml`. Fix the neo4j tag (and audit `ollama/ollama:0.22.1`) in both `docker-compose.yml` and `ci.yml`; each must be pinned, ≥14 days old, and pass Trivy. _May force a CVE-acceptance decision on the Neo4j image._

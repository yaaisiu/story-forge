...
root stays focused on top-level artifacts (`README.md`, `CLAUDE.md`, the spec,
`docker-compose.yml`, `LICENSE`, `SECURITY.md`). All references in the root
`CLAUDE.md`, `README.md`, and the `.claude/skills/` files now point at
`docs/PLAN_*.md`.
...
2. **Read before starting** — every doc the next session needs *by path*:
   this file, the spec sections, the directory-level `CLAUDE.md`s. Be specific.

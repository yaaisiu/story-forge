...
## Aged-out cross-cutting items — routed here by the two-roll rule (Session 101 roll)

Ten items that sat on `docs/PLAN_SHORT.md`'s **Cross-cutting** list for **two or more milestone
rolls without being touched**, moved here verbatim at the Graph-quality roll under the two-roll rule
(`docs/AGENTS.md` §3, added the same session). Every one carried the escape hatch *"fix when the
relevant code is next touched"* — which the evidence says is not a forcing function: the code **was**
touched, repeatedly, and none of them moved. The error-mapper item below is the exemplar that earned
the rule — its own trigger is *"fold when one of these screens next needs an error-message change"*,
and when Session 96 needed exactly that, it **added the seventh copy** and updated the item's count
from 6 to 7.

Routing note: the rule offers a binary — **name it as a task in the incoming milestone**, or **move it
here**. The incoming milestone is deliberately **unchosen** at this roll (the fork is gated on running
fresh sample text end-to-end first), so there is no milestone to name a task in and every aged item
takes the second branch. That is not a demotion: per this file's own routing rule these get **reviewed
at the next milestone roll and promoted back to `PLAN_SHORT.md` when picked up** — which is the
reconsideration the standing carry-forward never actually gave them.

Text is verbatim as it stood in `PLAN_SHORT.md` (move, not delete — `docs/AGENTS.md` §4); the
`- [ ]` checkbox form and original dating are preserved so each item's age stays legible.

...
## Glossary (31 terms — see [[glossary]])
...
| Note | Type | What |
|---|---|---|
| [[graph-edge-evidence]] | proposal | **Graph-quality S3 — Edge evidence + verifiable merges (⚠ PROPOSED, register OPEN DM-EE-1..6 / OQ-31 — owner resolves before code)** — step-0 for `docs/specs/graph-quality.md` §3 **S3**. A **read/verify** slice: click an edge → predicate + source sentence(s); each entity-merge option shows a context quote + type + aliases; three cheap safeguards (gate exact-name dup-create; don't privilege a score-100 exact-name match; fix the amber "merge target" highlight on *New* cards). **S3 writes nothing** → INV-1/INV-3/INV-9 untouched; no egress/LLM; INV-4 constrains DM-EE-4 (no closed enum of "group" types). **Defining finding: the data is real but not on the wire** — edge [[provenance]] survives commit in `staged_relations` (keyed by the content-addressed `edge_id`) but reaches no client and has **no by-`edge_id` read/index** (`postgres_relation_store.py:94-112`); `GraphCanvas` taps **nodes only** (`GraphCanvas.tsx:106`); the merge surfaces show a bare `(100)` + generic "an existing entity" (`CandidateCard.tsx:75-81,131-132`, no `target_canonical_name`). So S3 needs a small set of new **read** surfaces (a focused edge-evidence BFF read + merge-context enrichment) — **not** new storage and **not** the §4 write-handle. Register: **DM-EE-1** delivery (focused per-edge BFF vs payload-enrich — *lean* BFF, the [[m4-side-panel]] DM-SP-1 precedent; provenance is one-to-many); **DM-EE-2** provenance source (all `written` `staged_relations` by `edge_id` + index vs the single lossy Neo4j property; zero-provenance = manually-added); **DM-EE-3** merge-context (type+aliases+quote+`target_canonical_name` — **closes the cross-cutting `CandidateView`-no-target-name item**); **DM-EE-4** the score-100 trap (context, *not* type-classification — INV-4); **DM-EE-5** dup-create gate (client-side warn-and-offer vs backend lookup); **DM-EE-6** slice cut (backend-first / frontend-second). Added [[provenance]] (glossary 30→31). **Handoff correction:** the §4 surrogate handle is **not** built or consumed here (S3 reads by the content id; the handle is an edge-*write* concern, S5/S6). **No spec amendment, no ADR anticipated.** |
...
30. **Graph-quality S2 BUILT (Session 73, PR #168) → S3 decomposed (2026-07-01, Session 74).** S2 shipped
    ...
    **S3 step-0 → [[graph-edge-evidence]]** (`status: proposed`, register **OPEN** DM-EE-1..6 / OQ-31). A
    **read/verify** slice (edge evidence + verifiable merges + three safeguards); **writes nothing**
    ...
    call **DM-EE-1** (focused per-edge read vs payload-enrich — *lean* BFF, the DM-SP-1 precedent). **No
    spec amendment / no ADR anticipated.** **Next:** the owner resolves DM-EE-1..6 (plain-language gaps in
    the proposal), then build S3 **backend-first, test-first** from the pure `staged_relations`-by-`edge_id`
    read + the enriched `CandidateView` projection.

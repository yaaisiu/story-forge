...
34. **Graph-quality S6 BUILT (S6a-1 #207 / S6a-2 #212 / S6b #214 / S6c #216, Sessions 92–97) → S7 BUILT
    (#222 + perf follow-up #223, Session 100) → the milestone is COMPLETE.** S6 shipped as four owner-split
    slices — read/suggest backend, the two graph-write apply ops, the `/stories/:id/normalise-names` list,
    then a precision tune of the label scorer (a subset-intolerant `label_match_score` at the *label* layer
    only, leaving the shared `name_match_score` untouched because the live extraction matcher wants subset
    tolerance). S6a-2's `relabel_entity_type` is INV-9's **eighth** witnessed writer-path and the one
    genuinely-new graph writer; the predicate rename became **INV-10's first realized consumer**. **S7 had no
    decompose** (not branchy) and its verify-first survey found the §3.5 correction surface already complete —
    so the slice closed a **spec drift** instead: §3.5 promised a tooltip "brief description" no entity has
    ever carried. Now a **graph-derived relation summary**, read-time, no stored field, no LLM — which
    **supersedes DM-IH-8** ([[m4-inline-highlights]]). **Next:** the milestone fork (deeper extraction work
    vs V2 Editing) is deliberately **unchosen** and gated on running fresh sample text end-to-end first — the
    working graph has been hand-curated across S4–S7 and no longer shows what the pipeline produces.
35. **Milestone-roll sweep (2026-07-23, Session 101) → [[2026-07-23-architecture-review]].** The first
    `review-architecture` run since Session 90's decompose, and the first at a *milestone roll* under the
    ADR-0002 §4 wiring. Headline: [[overview]] and [[project]] were a **whole milestone stale** (both still
    said "now in Public-readiness"), so the entire Graph-quality as-built record was missing. Re-synced both,

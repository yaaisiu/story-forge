...
32. **Graph-quality S5 BUILT (S5a #185 / S5b-be #189 / S5b-fe #191, Sessions 81–83) + out-of-sequence
    ...
    **re-keys** every bearing edge, reusing the S5b `plan_relation_rekey`, preserving `edge_uid` per INV-10,
    ...
    amendment, no ADR anticipated.** **Next:** the owner resolves DM-NN-1..6 (one plain-language question at a
    time), then build S6 test-first from the shared pure suggest function — a fresh conversation.
